...
class AnyDeviceManager(gatt.DeviceManager):
    def __init__(self, adapter_name, duration):
    ...
    def device_discovered(self, device):
        # self.devices[device.mac_address] = AnyDevice(mac=device.mac_address, manager=self)
        self.devices.add(device.mac_address)
        if time.time() >= self.start + self.duration:
            self.stop()


def is_homekit_accessory(device):
    for service in device.services:
        if service.uuid == ServicesTypes.get_uuid(ServicesTypes.PAIRING_SERVICE):
            return True
    return False
...
manager = AnyDeviceManager(adapter_name='hci0', duration=5)
...
print('discovered', len(manager.devices))

for mac in list(manager.devices):
    manager = gatt.DeviceManager(adapter_name='hci0')
    dev = AnyDevice(manager=manager, mac_address=mac)
    dev.connect()
    if dev.is_connected():
        manager.run()
    if not is_homekit_accessory(dev):
        print('skipping ', dev.mac_address)
        continue
    print('-' * 80)
    print(mac, len(dev.services))

    if len(dev.services) > 0:
        handle_services(dev.services)
    # manager.devices[mac].connect()
    # print(mac, manager.devices[mac].services)
